...
plot4 <- function(power_df,project_folder) {
    plot_file <- file.path(project_folder,paste("plot4.png",sep=""))
    png(file=plot_file,width=480, height=480,units="px")
    par(mfrow = c(2, 2), mar = c(4,4,2,1), oma = c(0,0,2,0)) 
    plot(power_df$Time,power_df$Global_active_power, type="l", xlab="", ylab="Global Active Power")
    plot(power_df$Time,power_df$Voltage, type="l", xlab="datetime", ylab="Voltage")
    plot(power_df$Time,power_df$Sub_metering_1, type="l", xlab="", ylab="Energy sub metering")
    ...
    plot(power_df$Time,power_df$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power")
...
# plot dataframe in project folder
    ...
    plot_ds <- plot4(power_data_subset,project_folder_name)
    log(script_r,"[#5] ",plot_ds," complete and written to project folder")
    
    log(script_r,"[#End] Project complete")
